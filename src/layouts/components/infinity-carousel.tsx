...
export const InfinityCarousel: React.FC<infinityCarouselProps> = ({
  ...
  const duplicatedPartners = [...children, ...children];
  ...
  useEffect(() => {
    const startAnimation = () => {
      controls.start({
        x: -contentWidthToScroll, // Animate from 0 to negative of content width
        transition: {
          x: {
            duration: 40, // Match original animation duration
            ease: 'linear',
            repeat: Infinity,
            repeatType: 'loop',
          },
